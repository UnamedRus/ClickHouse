...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.6",
        {
            ...
            {"object_storage_identity_cache_fetch_part_offsets", false, false, "New setting: fetch the S3 multipart part layout via GetObjectAttributes when reading object metadata (for part-aligned reads). Disabled by default, in which case a cheaper HEAD (size + etag) is used, avoiding a per-file GetObjectAttributes on cold lake scans."},
            ...
            {"input_format_parquet_fill_constant_pages", false, false, "New experimental setting: fill single-valued Parquet data pages from per-page Column Index statistics instead of decoding them, even when only part of a row subgroup is constant. Disabled by default."},
            {"input_format_parquet_align_reads_to_multipart_boundaries", false, false, "New experimental setting: align coalesced Parquet read requests to S3 multipart-upload part boundaries so a single read never straddles two parts. Disabled by default."},
            {"input_format_parquet_read_alignment_bytes", 0, 0, "New experimental setting: align coalesced Parquet reads to a fixed byte grid (e.g. the writer's multipart part size) without probing per-file layout. 0 disables."},
            {"input_format_parquet_read_alignment_min_bytes", 1048576, 1048576, "New experimental setting: anti-fragmentation guard for Parquet read alignment - do not split a read at a boundary when the aligned segment would be smaller than this. Default 1 MiB."},
            {"input_format_parquet_split_reads_across_part_boundaries", false, false, "New experimental setting: split a boundary-straddling Parquet read into per-part segments fetched in parallel (a straddling S3 GET pays ~one extra round-trip), and enable the speculative-parallel footer read. Disabled by default."},
            {"input_format_parquet_hedged_read_threshold_ms", 0, 0, "New experimental setting: hedge a Parquet read (issue a duplicate, take the winner) if it exceeds this many ms, to cut the S3 GET tail latency. 0 disables."},
            {"input_format_parquet_hedged_read_ttfb_threshold_ms", 0, 0, "New experimental setting: hedge a Parquet read when the primary's first byte (TTFB) has not arrived within this many ms, rather than when the whole read exceeds a budget - separates a stalled connection from a slow-but-progressing large transfer. Takes precedence over input_format_parquet_hedged_read_threshold_ms. 0 disables."},
            {"input_format_parquet_read_min_fill_ratio", 0., 0., "New experimental setting: anti-amplification guard for Parquet v3 read coalescing - stop bridging gaps once a coalesced task would be less than this fraction wanted bytes, so a tiny scattered column no longer drags in its neighbours. 0 disables."},
            {"input_format_parquet_hedged_read_max_bytes", 4194304, 4194304, "New experimental setting: only hedge Parquet reads no larger than this many bytes. Default 4 MiB."},
            {"input_format_parquet_hedged_read_max_inflight", 4, 4, "New experimental setting: cap on concurrent hedged Parquet reads. Default 4."},
